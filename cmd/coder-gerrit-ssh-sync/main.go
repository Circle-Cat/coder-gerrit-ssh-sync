...
func syncUser(ctx context.Context, client *coderclient.CoderClient, gClient gerritAccountService, user *coderclient.CoderUser) error {
	...
	log.Printf("Syncing user %q", user.FormatCoderUser())
	...
	log.Printf("Got Git SSH key for user %q: %s", user.FormatCoderUser(), key.PublicKey)

	var errs []error
	for _, gu := range *gus {
		log.Printf("Got Gerrit user AccountID %d for Coder user %q", gu.AccountID, user.FormatCoderUser())
